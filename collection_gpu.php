...
<main class="max-w-7xl mx-auto px-4 py-8">
  ...
  <form class="mb-4" method="get" action="">
    <input class="border rounded-lg p-2 w-64" name="q" placeholder="例: RTX 4070 / RTX 4090" value="<?=h($q)?>">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
    <?php foreach($list as $p): ?>
      <article class="rounded-2xl border bg-white p-4">
        <div class="badge mb-2"><?=h($p['gpu'])?></div>
        <h3 class="font-semibold"><?=h($p['name'])?></h3>
        <p class="text-sm text-gray-600"><?=h($p['cpu'])?> / <?=h($p['ram'])?>GB</p>
